		...
		if ($('#modal-' + videoId).length != 0) {
			if ($('#modal-' + videoId).css('display') == 'none') {
				document.getElementById('YTmodal-' + videoId).innerHTML =
					"<iframe id='ytplayer' style='height: 90vh; width: 90vw;' src='" +
					srcValue +
					"' allowfullscreen></iframe>";
				document
					.getElementById('modal-' + videoId)
					.setAttribute(
						'style',
						'position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: rgba(140, 140, 140, 0.3); z-index: 10000;'
					);
			} else {
				document.getElementById('YTmodal-' + videoId).innerHTML = '';
				document
					...
					.setAttribute(
						'style',
						'display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: rgba(140, 140, 140, 0.3); z-index: 10000;'
					);
			}
		} else {
